import { LayoutDashboard, Car, Wrench, MapPin, Users, CalendarSync, Settings } from 'lucide-react';
...
const OperationalAdminSidebar = () => {
    return (
        <aside className="w-64 h-full flex flex-col flex-shrink-0" style={{ background: '#0A0A0A', borderRight: '1px solid #2A2A2A' }}>
            ...
            <div className="px-5 py-4 text-xs font-bold text-gray-500 tracking-wider">L2: OPS ADMIN</div>

            <div className="flex-1 overflow-y-auto px-3">
                {[
                    { icon: <LayoutDashboard size={20} />, label: 'Ops Overview', active: true },
            ...
            <div className="p-4 border-t" style={{ borderColor: '#2A2A2A' }}>
                <div className="flex items-center gap-3 cursor-pointer text-gray-400 hover:text-white transition-colors p-2 rounded-lg hover:bg-white/5">
                    <Settings size={20} />
                    <span className="text-sm font-medium">Ops Settings</span>
                </div>
            </div>
        </aside>
    );
