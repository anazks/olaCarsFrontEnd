import { LayoutDashboard, Map, FileCheck, Building2, TrendingUp, AlertOctagon, Settings } from 'lucide-react';
...
const CountryManagerSidebar = () => {
    return (
        <aside className="w-64 h-full flex flex-col flex-shrink-0" style={{ background: '#0A0A0A', borderRight: '1px solid #2A2A2A' }}>
            ...
            <div className="px-5 py-4 text-xs font-bold text-gray-500 tracking-wider">L2: COUNTRY DIR</div>

            <div className="flex-1 overflow-y-auto px-3">
                {[
                    { icon: <LayoutDashboard size={20} />, label: 'National Dashboard', active: true },
                    { icon: <Map size={20} />, label: 'Regional Performance' },
                    { icon: <Building2 size={20} />, label: 'Branch Directory' },
                    { icon: <TrendingUp size={20} />, label: 'Revenue & Growth' },
                    { icon: <FileCheck size={20} />, label: 'Compliance Reports' },
                    { icon: <AlertOctagon size={20} />, label: 'Critical Escalations' },
                ].map((item, i) => (
                    <div
                        key={i}
