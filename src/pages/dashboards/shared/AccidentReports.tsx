...
const AccidentReports = () => {
    ...
    return (
        ...
        >
            <div 
                className="px-6 md:px-8 pt-4 border-b pb-3"
            ...
            {/* Compact Header */}
            <div 
                className="px-6 md:px-8 py-4 border-b flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 z-30"
                style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-main)' }}
            >
                <div>
                    <h1 className="text-lg font-bold tracking-tight flex items-center gap-2 uppercase italic" style={{ color: 'var(--text-main)' }}>
                        <ShieldAlert size={20} className="text-red-500" /> Incident <span className="text-red-500">Command</span>
                    </h1>
                    <p className="text-xs font-medium mt-0.5" style={{ color: 'var(--text-dim)' }}>Live Fleet Incident & Accident Monitoring</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                    <div className="relative group min-w-[220px]">
                        ...
                        <input 
                            ...
                            className="w-full border rounded-xl pl-9 pr-3 py-2 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-red-500/20 transition-all"
                    ...
                    <select 
                        ...
                        className="border rounded-xl px-3 py-2 text-[11px] font-bold focus:outline-none cursor-pointer appearance-none"
                        style={{ backgroundColor: 'var(--bg-main)', borderColor: 'var(--border-main)', color: 'var(--text-main)' }}
                    >
                    ...
                    <button 
                        ...
                        className="flex items-center justify-center p-2 rounded-xl border hover:text-red-500 hover:border-red-500/30 transition-all"
                            ...
                            {[
                                ...
                                >
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        <span className="text-2xl font-black" style={{ color: 'var(--text-main)' }}>{stat.value}</span>
                                    </div>
                                    <p className="text-[10px] font-black uppercase tracking-widest" style={{ color: 'var(--text-dim)' }}>{stat.label}</p>
                                </div>
                            ))}
                        </div>
                                ...
                                <table className="w-full text-left border-collapse min-w-[800px]">
                                    <thead>
                                        <tr className="bg-black/[0.01] dark:bg-white/[0.01]">
                                            <th className="px-8 py-5 text-xs font-bold uppercase tracking-wider border-b" style={{ borderColor: 'var(--border-main)', color: 'var(--text-dim)' }}>Incident & Asset</th>
                                            <th className="px-8 py-5 text-xs font-bold uppercase tracking-wider border-b" style={{ borderColor: 'var(--border-main)', color: 'var(--text-dim)' }}>Location Info</th>
                                            <th className="px-8 py-5 text-xs font-bold uppercase tracking-wider border-b" style={{ borderColor: 'var(--border-main)', color: 'var(--text-dim)' }}>Status</th>
                                            <th className="px-8 py-5 text-xs font-bold uppercase tracking-wider border-b text-right" style={{ borderColor: 'var(--border-main)', color: 'var(--text-dim)' }}>Review</th>
                                        </tr>
                                    </thead>
                                        ...
                                        {loading ? (
                                            [1, 2, 3, 4, 5].map(i => (
                                                <tr key={i} className="animate-pulse">
                                                    <td colSpan={4} className="px-8 py-8">
                                                        <div className="h-4 rounded-full w-full" style={{ backgroundColor: 'var(--bg-main)' }} />
                                                    </td>
                                                </tr>
                                            ))
                                            ...
                                            <tr>
                                                <td colSpan={4} className="p-32 text-center">
                                                    <div className="flex flex-col items-center gap-4 opacity-30">
                                                        ...
                                                        <p className="text-sm font-black uppercase tracking-[0.3em]" style={{ color: 'var(--text-main)' }}>No incidents recorded</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        ) : (
                                            filteredReports.map((r) => (
                                                <tr 
                                                    ...
                                                    className="group transition-all cursor-pointer relative hover:bg-[var(--sidebar-hover)]"
                                                >
                                                    <td className="px-8 py-5">
                                                        <div className="flex items-center gap-5">
                                                            <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 shadow-sm transition-transform group-hover:scale-105 ${r.status === 'SUBMITTED' ? 'bg-red-500 text-white' : 'bg-black/5 dark:bg-white/5 text-gray-400'}`}>
                                                                 <Car size={18} />
                                                            </div>
                                                            <div>
                                                                <div className="flex items-center gap-2 mb-0.5">
                                                                    <p className="text-sm font-black uppercase tracking-tight" style={{ color: 'var(--text-main)' }}>{r.vehicleNumber}</p>
                                                    ...
                                                    <td className="px-8 py-5 text-right">
                                                        <button 
                                                            className="p-2.5 rounded-xl text-gray-400 group-hover:text-red-500 group-hover:bg-red-500/10 transition-all border border-transparent group-hover:border-red-500/20"
