...
const EditFinanceStaff = () => {
    ...
    const loadData = async (staffId: string) => {
        ...
        setError(null);
        try {
            const data = await getFinanceStaffById(staffId);
            setFormData({
                id: data._id,
                ...
                status: data.status,
                fleetNumbers: Array.isArray(data.fleetNumbers) ? data.fleetNumbers.join(', ') : '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const payload: UpdateFinanceStaffPayload = {
                ...
                fleetNumbers: formData.fleetNumbers ? formData.fleetNumbers.split(',').map(s => s.trim()).filter(Boolean) : []
            };
    ...
    return (
        <div className="space-y-6 animate-fadeInUp">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold" style={{ color: 'var(--text-main)' }}>Edit Finance Staff</h1>
                    ...
                    {activeTab === 'details' ? (
                        <div className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1.5">
                                    <label className="block text-sm font-medium" style={{ color: 'var(--text-dim)' }}>Branch</label>
                                    <select required value={formData.branchId} onChange={(e) => setFormData({ ...formData, branchId: e.target.value })} className="w-full px-4 py-3 rounded-xl outline-none text-sm focus:ring-2 focus:ring-lime appearance-none cursor-pointer" style={{ background: 'var(--bg-input)', border: '1px solid var(--border-main)', color: 'var(--text-main)' }}>
                                        <option value="">Select Branch</option>
                                        {branches.map(b => <option key={b._id} value={b._id}>{b.name} ({b.code})</option>)}
                                    </select>
                                </div>
                                <div className="space-y-1.5 md:col-span-2">
                                    <label className="block text-sm font-medium" style={{ color: 'var(--text-dim)' }}>Fleet Numbers</label>
                                    <input type="text" value={formData.fleetNumbers} onChange={(e) => setFormData({ ...formData, fleetNumbers: e.target.value })} placeholder="e.g. 101, 102, 103" className="w-full px-4 py-3 rounded-xl outline-none text-sm focus:ring-2 focus:ring-lime" style={{ background: 'var(--bg-input)', border: '1px solid var(--border-main)', color: 'var(--text-main)' }} />
                                </div>
                            </div>
                        </div>
                    ) : (
...
export default EditFinanceStaff;
