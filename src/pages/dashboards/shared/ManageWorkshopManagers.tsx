...
const ManageWorkshopManagers = () => {
  ...
  const [selectedManager, setSelectedManager] =
    useState<WorkshopManager | null>(null);
  ...
  // Delete confirmation
  const [deleteTarget, setDeleteTarget] = useState<WorkshopManager | null>(
    null,
  );
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          err.message ||
          t("management.workshopManager.fetchFailed", {
  ...
  const handleUnblock = async (staff: any) => {
    try {
      await updateWorkshopManager({
        id: staff._id,
        status: "ACTIVE",
      });
      fetchData();
  ...
  const SortIcon = ({ field }: { field: string }) => {
    if (sortBy !== field)
      return (
        <div className="opacity-20 transition-opacity group-hover:opacity-50">
          <ChevronDown size={14} />
        </div>
      );
    return (
  ...
  const FilterLabel = ({ label }: { label: string }) => (
    <label
      className="block text-[10px] uppercase font-black tracking-widest mb-1.5 ml-1"
    ...
    >
      <Breadcrumbs
        items={[
          ...
          { label: "Manage Workshop Managers", active: true },
        ]}
      />
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          >
            <UserCog
              ...
              style={{ color: "var(--brand-lime)" }}
            />
            {t("management.workshopManager.title", {
              defaultValue: "Workshop Managers",
            })}
          </h1>
          <p
            ...
            style={{ color: "var(--text-dim)" }}
          >
            {t("management.workshopManager.subtitle", {
              defaultValue: "Manage workshop managers across branches",
            })}
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button
            ...
            title={t("management.common.refreshData", {
          ...
          <HasPermission permission="STAFF_CREATE">
            ...
            >
              <Plus size={20} />{" "}
              {t("management.workshopManager.add", {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
        ...
        >
          <p
            className="text-xs uppercase font-black tracking-widest transition-colors mb-1"
            style={{ color: "var(--text-dim)" }}
          >
            {t("management.common.stats.totalManagers", {
              defaultValue: "Total Managers",
            })}
          </p>
          <h3
        ...
        >
          <p
            ...
            style={{ color: "var(--text-dim)" }}
          >
            {t("management.common.stats.activeAccounts", {
              defaultValue: "Active Accounts",
            })}
          </p>
          <h3
            className="text-3xl font-black text-lime transition-colors"
            style={{ color: "var(--brand-lime)" }}
          >
            {workshopManagers.filter((s) => s.status === "ACTIVE").length}
          </h3>
        </div>
        <div
          className="p-5 rounded-2xl border transition-colors"
        ...
        >
          <p
            ...
            style={{ color: "var(--text-dim)" }}
          >
            {t("management.common.stats.totalBranches", {
      ...
      {/* Toolbar / Filters */}
      <div
        className="p-6 rounded-2xl border mb-8 space-y-4 transition-colors"
        style={{
          background: "var(--bg-card)",
          borderColor: "var(--border-main)",
        }}
      >
        ...
        {showAdvancedFilters && (
          <div
            className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t transition-all animate-in slide-in-from-top-2 duration-300"
          ...
          >
            <div>
              ...
              >
                <option value="ALL">
                  {t("management.common.allStatuses")}
                </option>
                <option value="ACTIVE">
                  {t("management.common.status.active")}
                </option>
                <option value="SUSPENDED">
                  {t("management.common.status.suspended", {
                    defaultValue: "Suspended",
                  })}
                </option>
                <option value="LOCKED">
                  {t("management.common.status.locked", {
                    defaultValue: "Locked",
                  })}
                </option>
              </select>
            ...
            <div>
              <FilterLabel
                label={t("management.common.joinedDateFrom", {
                  defaultValue: "Joined From",
                })}
              />
              <input
                type="date"
            ...
            <div>
              <FilterLabel
                label={t("management.common.joinedDateTo", {
                  defaultValue: "Joined To",
                })}
              />
              <input
                type="date"
      ...
      {/* Table */}
      <div
        className="rounded-2xl border overflow-hidden transition-colors shadow-sm"
        style={{
          background: "var(--bg-card)",
          borderColor: "var(--border-main)",
        }}
      >
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr
                className="border-bottom text-[10px] uppercase font-black tracking-wider transition-colors"
                style={{
              ...
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center">
                    <RefreshCw
                      size={24}
                      className="animate-spin inline-block mb-2 text-lime"
                    />
                    <p className="text-sm" style={{ color: "var(--text-dim)" }}>
                      {t("management.workshopManager.loading", {
                        defaultValue: "Loading workshop managers...",
                      })}
                    </p>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center">
                    <p
                      ...
                      style={{ color: "var(--text-main)" }}
                    >
                      {t("management.workshopManager.notFound", {
                        defaultValue: "No Workshop Managers found",
                      })}
                    </p>
                  </td>
                </tr>
                ...
                filteredManagers.map((manager) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        {t(
                          `management.common.status.${manager.status.toLowerCase()}`,
                        )}
                      </span>
                    </td>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => {
                            ...
                            navigate(
                              `${basePath}/staff-performance/${manager._id}`,
                            );
                          }}
                        ...
                        <HasPermission permission="STAFF_DELETE">
                          <button
                            ...
                            title={t("management.common.delete.title", {
                              defaultValue: "Delete",
                            })}
                          >
                            <Trash2 size={18} />
        ...
        {pagination && pagination.totalPages > 1 && (
          <div
            className="px-6 py-4 border-t flex flex-col sm:flex-row items-center justify-between gap-4 transition-colors"
            style={{
              borderColor: "var(--border-main)",
                    ...
                    style={{
                      background:
                        currentPage === i + 1
                          ...
                          : "transparent",
                      color:
                        currentPage === i + 1 ? "#000" : "var(--text-main)",
                      border:
                        currentPage === i + 1
                          ? "none"
      ...
      {error && (
        <div className="fixed bottom-6 right-6 flex items-center gap-3 px-6 py-4 rounded-2xl bg-red-500 text-white shadow-2xl animate-slide-up z-[100]">
          ...
          <button
            ...
            className="ml-4 hover:scale-110 active:scale-95 transition-transform"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2
                  className="text-lg font-black transition-colors uppercase tracking-tight"
                  style={{ color: "var(--text-main)" }}
                >
                  {modalMode === "create"
                    ? t("management.workshopManager.modalTitleCreate", {
                        defaultValue: "Add Workshop Manager",
                      })
                    : t("management.workshopManager.modalTitleEdit", {
                        defaultValue: "Edit Workshop Manager",
                      })}
                </h2>
                <p className="text-sm" style={{ color: "var(--text-dim)" }}>
                  {modalMode === "create"
                    ? t("management.workshopManager.subtitle", {
                        defaultValue: "Create a new manager account",
                      })
                    : t("management.workshopManager.modalTitleEdit", {
                        defaultValue: "Edit Workshop Manager",
                      })}
            ...
            {/* Tabs */}
            <div
              className="flex gap-4 border-b mb-6 transition-colors"
              style={{ borderColor: "var(--border-main)" }}
            >
              <button
                type="button"
              ...
              >
                {t("management.common.tabs.details", {
              ...
              >
                {t("management.common.tabs.permissions", {
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        {modalMode === "create"
                          ...
                          : t("management.common.modal.newPasswordOptional", {
                      ...
                      {modalMode === "edit" && (
                        <p className="text-[11px] text-slate-400 mt-1">
                          {t("management.common.modal.keepPasswordInfo", {
                            defaultValue:
              ...
              <div className="pt-4 flex items-center justify-end gap-3">
                ...
                >
                  {formLoading ? (
                    ...
                    t("management.workshopManager.createButton", {
                      defaultValue: "Create Manager",
                    })
                  ) : (
                    t("management.workshopManager.updateButton", {
                      defaultValue: "Update Manager",
                    })
                  )}
                </button>
              </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          >
            ...
            <h2
              className="text-xl font-black mb-2 transition-colors uppercase tracking-tight"
              style={{ color: "var(--text-main)" }}
            >
              {t("management.workshopManager.deleteTitle", {
                defaultValue: "Delete Manager",
              })}
            </h2>
            <p
              ...
              style={{ color: "var(--text-dim)" }}
            >
              {t("management.workshopManager.deleteConfirm", {
                defaultValue: "Are you sure you want to delete this manager?",
