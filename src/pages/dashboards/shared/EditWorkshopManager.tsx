...
const EditWorkshopManager = () => {
    ...
    const loadData = async (managerId: string) => {
        setLoading(true);
        setError(null);
        try {
            const data = await getWorkshopManagerById(managerId);
            setFormData({
                id: data._id,
                ...
                phone: data.phone || '',
                password: '',
                workshopId: typeof data.workshopId === 'object' && data.workshopId !== null ? (data.workshopId as any)._id : data.workshopId || '',
                status: data.status,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success('Workshop Manager updated successfully');
            navigate(-1);
        } catch (err: any) {
    ...
    return (
        <div className="space-y-6 animate-fadeInUp p-4 sm:p-6" style={{ background: 'var(--bg-main)' }}>
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold" style={{ color: 'var(--text-main)' }}>Edit Workshop Manager</h1>
                    ...
                    {activeTab === 'details' ? (
                        <div className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1.5">
                                    <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--text-dim)' }}>Phone</label>
                                    <PhoneInput
                                        country={'in'}
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <select required value={formData.workshopId} onChange={(e) => setFormData({ ...formData, workshopId: e.target.value })} className="w-full px-4 py-3 rounded-xl outline-none text-sm focus:ring-2 focus:ring-lime appearance-none cursor-pointer" style={{ background: 'var(--bg-input)', border: '1px solid var(--border-main)', color: 'var(--text-main)' }}>
                                        ...
                                        {workshops.map(w => <option key={w._id} value={w._id}>{w.name} ({w.code})</option>)}
                                    </select>
                                </div>
                            </div>
                        </div>
...
export default EditWorkshopManager;
