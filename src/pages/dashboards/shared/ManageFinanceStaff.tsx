...
import { Plus, Pencil, Trash2, X, RefreshCw, Search, Mail, Phone, Building2, AlertTriangle, ChevronDown, Filter, ChevronLeft, ChevronRight, ShieldCheck, Eye, Unlock } from 'lucide-react';
import {
    getAllFinanceStaff,
    createFinanceStaff,
    updateFinanceStaff,
...
const ManageFinanceStaff = () => {
    ...
    const handleUnblock = async (staff: FinanceStaff) => {
        try {
            await updateFinanceStaff({
                id: staff._id,
                status: 'ACTIVE'
            });
            fetchData();
    ...
    return (
        <div className="p-4 sm:p-6 transition-colors duration-300" style={{ background: 'var(--bg-main)' }}>
            <Breadcrumbs items={[{ label: 'Dashboard', path: '#' }, { label: 'Manage Finance Staff', active: true }]} />
