        ...
        <div className="p-6 max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            ...
                            <span className="px-3 py-1 bg-brand-lime/10 text-brand-lime text-xs font-bold rounded-full border border-brand-lime/20 uppercase tracking-wider">
                                {driver.status.replace(/_/g, ' ')}
                            </span>
                        </div>
                        <p className="text-gray-500 flex items-center gap-2 text-sm mt-1">
                            <FileText size={14} />
                            License: <span className="font-bold text-gray-700">{driver.licenseNumber}</span>
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button className="px-6 py-2.5 bg-white border border-red-200 text-red-600 font-bold rounded-xl hover:bg-red-50 transition-all flex items-center gap-2">
                        <Ban size={18} />
                        Disqualify
                    </button>
                </div>
                            ...
                            <div key={step.id} className="relative z-10 flex flex-col items-center">
                                <div
                                    className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${isCompleted ? 'bg-brand-lime border-brand-lime text-black' :
                                        isCurrent ? 'bg-white border-brand-lime text-brand-lime shadow-lg shadow-brand-lime/20' :
                                ...
                                <div className="mt-3 text-center">
                                    <div className={`text-xs font-bold uppercase tracking-wider ${isCurrent ? 'text-black' : 'text-gray-400'}`}>{step.label}</div>
                                    {!isCurrent && <div className="text-[10px] text-gray-300 font-medium uppercase mt-0.5">{step.sub}</div>}
                                    {isCurrent && <div className="text-[10px] text-brand-lime font-bold uppercase mt-0.5 animate-pulse">In Progress</div>}
                                </div>
                            </div>
                        );
                    })}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 overflow-hidden relative">
                        <div className="absolute top-0 right-0 w-24 h-24 bg-brand-lime/5 rounded-bl-[100px] -mr-8 -mt-8" />
                        <div className="flex items-center gap-2 mb-6 border-b border-gray-50 pb-4 relative z-10">
                            <div className="p-2 bg-brand-lime/10 rounded-lg text-brand-lime">
                                <User size={20} />
                            </div>
                            <h2 className="font-bold text-gray-900 uppercase tracking-widest text-sm">Personal Details</h2>
                        </div>
                    ...
                    {/* Credit Check Status */}
                    {driver.creditCheck && (
                        <div className={`p-6 rounded-2xl shadow-sm border transition-all ${driver.creditCheck.decision === 'APPROVE' ? 'bg-green-50/50 border-green-100' :
                            driver.creditCheck.decision === 'DECLINE' ? 'bg-red-50/50 border-red-100' :
                                'bg-blue-50/50 border-blue-100'
                            }`}>
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    <div className={`p-2 rounded-lg ${driver.creditCheck.decision === 'APPROVE' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                        <CreditCard size={20} />
                                    </div>
                                    <h3 className="font-bold text-gray-900 uppercase tracking-widest text-sm">Credit Assessment</h3>
                                </div>
                                <span className={`px-3 py-1 rounded-full text-xs font-bold border ${driver.creditCheck.decision === 'APPROVE' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
                            ...
                            <div className="grid grid-cols-3 gap-6">
                                <div className="space-y-1">
                                    ...
                                    <p className="font-bold text-xl text-gray-900">{driver.creditCheck.score || 'N/A'}</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Status</p>
                                    <p className="font-bold text-gray-900 mt-1">{driver.creditCheck.status}</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Assessment Date</p>
                                    <p className="font-bold text-gray-900 mt-1">{driver.creditCheck.performedAt ? new Date(driver.creditCheck.performedAt).toLocaleDateString() : 'N/A'}</p>
                                </div>
                            </div>
                        </div>
                    )}
                ...
                <div className="space-y-6">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-full">
                        <div className="flex items-center gap-2 mb-6 border-b border-gray-50 pb-4">
                            ...
                            <h2 className="font-bold text-gray-900 uppercase tracking-widest text-sm">Required Docs</h2>
                        </div>
...
const InfoCard = ({ label, value, icon }: { label: string; value: any; icon?: React.ReactNode }) => (
    <div className="space-y-1 group">
        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider group-hover:text-brand-lime transition-colors">{label}</label>
        <div className="flex items-center gap-2 font-bold text-gray-900 group-hover:translate-x-1 transition-transform">
...
const DocUploadRow = ({ label, status, url, uploading, onUpload }: { 
    ...
    <div className="p-4 bg-gray-50/50 border border-gray-100 rounded-xl group hover:border-brand-lime/30 transition-all">
        ...
        {url ? (
            ...
            <div className="relative">
                ...
                <button
                    disabled={uploading}
                    className="w-full flex items-center justify-center gap-2 py-2 border border-dashed border-gray-200 rounded-lg text-xs font-bold text-gray-500 hover:border-brand-lime hover:text-brand-lime transition-all"
                >
