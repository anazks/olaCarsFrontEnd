...
const EditFinancialAdmin = () => {
    ...
    const loadAdmin = async (adminId: string) => {
        ...
        setError(null);
        try {
            const data = await getFinancialAdminById(adminId);
            setFormData({
                id: data._id,
            ...
            setError(err.response?.data?.message || 'Failed to load financial admin');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const payload: UpdateFinancialAdminPayload = {
                ...
                status: formData.status as 'ACTIVE' | 'SUSPENDED' | 'LOCKED',
            ...
            toast.success('Financial Admin updated successfully');
            navigate(-1);
        } catch (err: any) {
    ...
    return (
        <div className="space-y-6 animate-fadeInUp">
            <div className="flex items-center gap-4">
                <button
                    onClick={() => navigate(-1)}
                    className="p-2 rounded-xl hover:bg-white/5 transition-colors border"
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex gap-3 pt-4 border-t" style={{ borderColor: 'var(--border-main)' }}>
                        <button
                            type="button"
                            onClick={() => navigate(-1)}
                            className="px-6 py-3 rounded-xl text-sm font-medium cursor-pointer transition-all hover:bg-white/5 border"
                        ...
                        <button
                            type="submit"
                            disabled={saving}
                            className="px-8 py-3 rounded-xl text-sm font-bold cursor-pointer transition-all flex items-center justify-center disabled:opacity-60 hover:shadow-lg hover:-translate-y-0.5"
...
export default EditFinancialAdmin;
