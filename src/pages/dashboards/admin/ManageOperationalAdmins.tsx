...
const ManageOperationalAdmins = () => {
    ...
    const fetchAdmins = useCallback(async () => {
        ...
        try {
            ...
            setError(err.response?.data?.message || err.message || t('management.opAdmins.fetchFailed', { defaultValue: 'Failed to fetch operational admins' }));
        } finally {
            setLoading(false);
        }
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold flex items-center gap-3 transition-colors" style={{ color: 'var(--text-main)' }}>
                        ...
                        {t('management.opAdmins.title')}
                    </h1>
                    <p className="text-sm mt-1 transition-colors" style={{ color: 'var(--text-dim)' }}>{t('management.opAdmins.subtitle')}</p>
                </div>
                <div className="flex gap-3">
                    ...
                    >
                        <Plus size={18} /> {t('management.opAdmins.add')}
                    </button>
                </div>
            </div>
            ...
            <div className="rounded-2xl overflow-x-auto transition-colors" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-main)' }}>
                {loading ? (
                    ...
                    <div className="text-center py-20 transition-colors" style={{ color: 'var(--text-dim)' }}>
                        ...
                        <p className="text-lg font-medium">{t('management.opAdmins.notFound')}</p>
                        <p className="text-sm mt-1">{t('management.opAdmins.clickToAdd')}</p>
                    </div>
                ) : (
                    ...
                    >
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold transition-colors" style={{ color: 'var(--text-main)' }}>
                                {modalMode === 'create' ? t('management.opAdmins.modalTitleCreate') : t('management.opAdmins.modalTitleEdit')}
                            </h2>
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                >
                                    {formLoading
                                        ...
                                        : modalMode === 'create' ? t('management.opAdmins.createButton') : t('management.common.modal.saveChanges')
                                    }
                                </button>
                            </div>
                        </form>
            ...
            {deleteTarget && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
                    ...
                    >
                        ...
                        <h3 className="text-lg font-bold transition-colors mb-2" style={{ color: 'var(--text-main)' }}>{t('management.opAdmins.deleteTitle')}</h3>
